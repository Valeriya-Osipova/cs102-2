...
def get_block(grid: List[List[str]], pos: Tuple[int, int]) -> List[str]:
    ...
    x = (pos[0] // 3) * 3
    y = (pos[1] // 3) * 3
    return [grid[x + i][y + j] for i in range(3) for j in range(3)]
...
def solve(grid: List[List[str]]) -> Optional[List[List[str]]]:
    ...
    if not position:
        return grid
    row, col = position
    for number in find_possible_values(grid, position):
        grid[row][col] = number
        solution = solve(grid)
        if solution:
            return solution
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for i in range(len(solution)):
        values_in_row = set(get_row(solution, (i, 0)))
        if values_in_row != set("123456789"):
            return False

    for j in range(len(solution)):
        values_in_col = set(get_col(solution, (0, j)))
